...
async function wallet(){const sb=window.__msGetSb?.()||window.sb,user=window.__msGetUser?.()||window.currentUser;if(!sb||!user)return null;const {data,error}=await sb.from('wallets').select('coins,gem').eq('user_id',user.id).maybeSingle();if(error)throw error;return data}
async function pay(cost){const sb=window.__msGetSb?.()||window.sb,user=window.__msGetUser?.()||window.currentUser,w=await wallet();if(!w||!sb||!user)throw new Error('로그인이 필요해.');const coins=Number(w.coins||0),gems=Number(w.gem||0);if(coins<cost.coinCost)throw new Error(`코인이 부족해. 필요 ${cost.coinCost.toLocaleString()}코인`);if(gems<cost.gemCost)throw new Error(`보석이 부족해. 필요 ${cost.gemCost.toLocaleString()}개`);const {error}=await sb.from('wallets').update({coins:coins-cost.coinCost,gem:gems-cost.gemCost}).eq('user_id',user.id);if(error)throw error}
async function payCoins(amount){const sb=window.__msGetSb?.()||window.sb,user=window.__msGetUser?.()||window.currentUser,w=await wallet();if(!w||!sb||!user)throw new Error('로그인이 필요해.');const coins=Number(w.coins||0);if(coins<amount)throw new Error(`코인이 부족해. 필요 ${amount.toLocaleString()}코인`);const {error}=await sb.from('wallets').update({coins:coins-amount}).eq('user_id',user.id);if(error)throw error}
function message(text,bad=false){const v=document.getElementById('equipmentView');if(!v)return;let e=v.querySelector('#eqCraftStatus');if(!e){e=document.createElement('div');e.id='eqCraftStatus';const c=v.querySelector('.eq-craft');c?.after(e)}e.textContent=text;e.style.cssText=`margin:8px 0 0;font-weight:800;color:${bad?'#b91c1c':'#166534'}`}
