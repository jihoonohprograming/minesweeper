...
const GEMS=[
 {id:'ruby',name:'루비',icon:'🔻'},
 {id:'sapphire',name:'사파이어',icon:'🔵'},
 {id:'topaz',name:'토파즈',icon:'🟠'},
 {id:'amethyst',name:'자수정',icon:'🟣'},
 {id:'onyx',name:'오닉스',icon:'⚫'},
 {id:'aquamarine',name:'아쿠아마린',icon:'💠'},
 {id:'emerald',name:'에메랄드',icon:'🟢'},
 {id:'garnet',name:'가넷',icon:'🔴'},
 {id:'jade',name:'제이드',icon:'🟢'},
 {id:'diamond',name:'다이아몬드',icon:'💎'}
];
const STAT_POOL=[
 ...
 {id:'event',min:5,max:35},
 ...GEMS.map(g=>({id:g.id,min:g.id==='diamond'?4:8,max:g.id==='diamond'?25:40}))
];
...
function makeItem(slot,material=''){
 const r=pickRarity(),quality=rnd(r.min,r.max),count=r.name==='전설'?6:r.name==='특별'?5:r.name==='희귀'?4:3;
 const pool=[...STAT_POOL].filter(s=>s.id!==material).sort(()=>Math.random()-.5).slice(0,Math.max(1,count-(material?1:0))),stats={};
 for(const s of pool){const base=rnd(s.min,s.max),q=.7+quality/200;stats[s.id]=Math.max(1,Math.round(base*r.m*q))}
 if(material){const g=STAT_POOL.find(s=>s.id===material)||{min:8,max:40};const q=.7+quality/200;stats[material]=Math.max(1,Math.round(rnd(g.min,g.max)*r.m*q))}
 return{id:'eq-'+Date.now()+'-'+Math.random().toString(36).slice(2,7),slot,rarity:r.name,code:codeFor(slot,r.name),quality,boostDays:rnd(5,20),stats,equipped:false,material:material||null}
}
...
function suite(){try{return JSON.parse(localStorage.getItem(SUITE_KEY)||'{}')||{}}catch(_){return{}}}
function saveSuite(s){localStorage.setItem(SUITE_KEY,JSON.stringify(s));window.dispatchEvent(new CustomEvent('mso-state-changed'))}
...
function consumeMaterial(material){if(!material)return;const s=suite();s.gems??={};const n=Number(s.gems[material]||0);const name=GEMS.find(g=>g.id===material)?.name||material;if(n<1)throw new Error(`${name}가 1개 필요해.`);s.gems[material]=n-1;saveSuite(s)}
function refundMaterial(material){if(!material)return;const s=suite();s.gems??={};s.gems[material]=Number(s.gems[material]||0)+1;saveSuite(s)}
...
async function craftPaid(){
 ...
 const material=document.getElementById('eqCraftGem')?.value||'';
 const item=makeItem(sel.value,material),cost=price(item),g=GEMS.find(x=>x.id===material),extra=g?` + ${g.icon} ${g.name} 1개`:'';
 if(!confirm(`이 장비의 제작 비용은 🪙 ${cost.coinCost.toLocaleString()}코인 + 💎 ${cost.gemCost.toLocaleString()}보석${extra}야.\n제작할까?`))return;
 let consumed=false;
 try{
  if(material){consumeMaterial(material);consumed=true}
  await pay(cost);
  const a=load();a.push(item);save(a);
  message(`✅ 제작 완료! ${g?`${g.name} 장비 · ${g.name} +${item.stats[material]}% · `:''}🪙 ${cost.coinCost.toLocaleString()} + 💎 ${cost.gemCost.toLocaleString()} 사용`);
  try{refreshProfile();refreshMarket()}catch(_){}
  setTimeout(()=>{document.querySelector('.nav-item[data-view="equipment"]')?.click()},50)
 }catch(e){if(consumed)refundMaterial(material);message(`❌ ${e?.message||e}`,true)}
}
async function upgradePaid(button){
 const card=button.closest('[data-eqid]'),id=card?.dataset.eqid,a=load(),it=a.find(x=>x.id===id);if(!it)return;
 const cost=upgradePrice(it);
 if(!confirm(`강화 비용은 🪙 ${cost.toLocaleString()}코인이야.\n현재 품질 ${it.quality}% → 제한 없이 계속 강화할 수 있어.\n강화할까?`))return;
 try{
  await payCoins(cost);
  it.quality=Number(it.quality||0)+rnd(1,4);
  for(const k of Object.keys(it.stats||{}))if(Math.random()<.45)it.stats[k]=Number(it.stats[k]||0)+1;
  save(a);message(`✅ 강화 완료! 🪙 ${cost.toLocaleString()}코인 사용 · 품질 ${it.quality}%`);
  try{refreshProfile();refreshMarket()}catch(_){}
...
function decorate(){
 decorateQueued=false;const v=document.getElementById('equipmentView');if(!v)return;
 const craft=v.querySelector('.eq-craft'),p=craft?.querySelector('p');
 if(p&&!p.dataset.costNote){p.dataset.costNote='1';p.textContent='일반 제작 또는 보석 제작을 선택할 수 있어. 보석으로 제작하면 그 보석의 +x% 효과가 반드시 붙고, 강화 횟수 제한은 없어.'}
 if(craft&&!craft.querySelector('#eqCraftGem')){
  const sel=document.createElement('select');sel.id='eqCraftGem';sel.innerHTML='<option value="">일반 제작</option>'+GEMS.map(g=>`<option value="${g.id}">${g.icon} ${g.name} 제작</option>`).join('');
  const slot=craft.querySelector('#eqCraftSlot');slot?.after(sel)
 }
 const items=load();v.querySelectorAll('[data-eqid]').forEach(card=>{const id=card.dataset.eqid,it=items.find(x=>x.id===id),btn=card.querySelector('.eq-upgrade');if(it&&btn){const text=`강화하기 (${upgradePrice(it).toLocaleString()} 코인)`;if(btn.textContent!==text)btn.textContent=text}})
}
